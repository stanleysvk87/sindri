...
from app.machines import MachineNameError, create_machine, delete_machine, list_machines
...
def add_machine(payload: MachineCreate):
    if payload.auth_type not in ("key", "password"):
    ...
    if payload.auth_type == "key" and not payload.ssh_key_path:
        raise HTTPException(status_code=400, detail="ssh_key_path is required for auth_type=key")
    if payload.auth_type == "key" and payload.ssh_key_path not in list_available_keys():
        ...
        raise HTTPException(
            status_code=400, detail="ssh_key_path is not one of the keys mounted from the host"
        )
    try:
